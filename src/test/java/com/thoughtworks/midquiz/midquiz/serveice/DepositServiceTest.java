...
import static com.thoughtworks.midquiz.midquiz.infraustruct.client.apimodel.AuctionStatus.RECALL;
import static com.thoughtworks.midquiz.midquiz.infraustruct.repo.po.BidAgreementStatus.PROCESSING;
...
class DepositServiceTest {
    ...
    void should_throw_exception_when_payment_client_throw_exception_given_deposit_info_is_wrong_and_auction_number(){
        ...
        when(paymentClientDegradation.getPaymentResponse(anyString())).thenThrow(new BusinessException("支付失败，请检查账户", HttpStatus.BAD_REQUEST));
    ...
    void should_return_payment_response_when_payment_client_throw_500_exception_given_scheduler_return_response(){
        ...
        when(bidAgreementRepository.findById(baid)).thenReturn(Optional.of(bidAgreement));
        when(auctionObjectClient.searchAuctionTarget(auctionNumber)).thenReturn(auctionObjectInfo);
        when(paymentClient.payment(any(PaymentRequest.class))).thenReturn(PaymentResponse.defaultFallBack(payNo));
        when(paymentClientDegradation.getPaymentResponse(anyString())).thenReturn(paymentResponse);
